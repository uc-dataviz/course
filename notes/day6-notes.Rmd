...
> A graphical form that involves elementary perceptual tasks that lead to more accurate judgments than another graphical form (with the same quantitiative information) will result in better organization and increase hte chances of a correct perception of patterns and behavior.
...
```{r pie}
plot_ly(diamonds_sum, labels = ~cut, values = ~pct, type = 'pie', sort = FALSE) %>%
...
ggplot(pie, aes(x = factor(1), y = value)) +
  ...
  geom_text(aes(label = label), position = position_stack(vjust = .5), size = 5) +
  coord_polar(theta = "y", direction = 1) +
  theme_void() +
  theme(legend.position = "none")

# table
knitr::kable(pie, col.names = c("", ""))
```
