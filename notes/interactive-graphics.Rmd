...
* Introduce `htmlwidgets`, `plotly`, `highcharter`, and `shiny` for generating interactive graphics in R
...
[Plotly](https://plot.ly/) is an online analytics and data visualization tool built using Python, JavaScript, and D3. They offer a commercial product for designing graphics via a point-and-click interface online, but have developed open-source packages for generating plotly graphs ([plotly.js](https://plot.ly/javascript/)), as well as API libraries for generating plotly graphs in [R](https://plot.ly/r/). The open-source JavaScript library is published under the MIT license.

## Generating `plot_ly()` objects

A plotly visualization is composed of one or more **trace(s)** (similar to a layer in `ggplot2`). `plot_ly()` objects can be built as piped operations if the object contains multiple traces, or a single call to `plot_ly()` if the object is relatively simple. These trace functions follow a `add_()` naming pattern. By default, `plot_ly()` assumes you want to draw a scatterplot.
...
* `add_markers()` - defines the appearance of the data points
...
* `add_lines()` - defines the line to be drawn. Here we have to generate it manually.
...
* [`plotly` for R](https://plotly-book.cpsievert.me/) - online book providing a more narrative introduction to `plotly` for R
* [`plotly` for R cheat sheet](https://images.plot.ly/plotly-documentation/images/r_cheat_sheet.pdf)

# `highcharter`

* Based on `highcharts` JavaScript library for interactive graphics
* Free for non-commercial use
* Paid license required for commercial and government usage
...
In this activity, we'll walk through all the steps of building a Shiny app using a dataset that lets you explore the products available at the [Virginia ABC stores](https://www.abc.virginia.gov/products).^[[Like many U.S. states](https://en.wikipedia.org/wiki/Alcohol_laws_of_the_United_States), Virginia exercises a monopoly over all sales of hard liquor in the state (plus an assortment of some wines).] The final version of the app [can be seen here](https://bensoltoff.shinyapps.io/cfss-abc/). Any activity deemed as an exercise throughout this tutorial is not mandatory for building our app, but they are good for getting more practice with Shiny. 
...
[![Shiny Run App](images/shiny-runapp.png)](images/shiny-runapp.png)
...
[![Shiny Stop App](images/shiny-stopapp.png)](images/shiny-stopapp.png)
...
The dataset we'll be using contains information about all the products sold by the Virginia ABC stores and was previously scraped by [taber](https://github.com/taber/Virginia-ABC-Data). The processed dataset we'll be using in this app is available [here](http://cfss.uchicago.edu/data/abc.csv). Download it now and place this file in the same folder as your Shiny app. Make sure the file is named `abc.csv`.

Add a line in your app to load the data into a variable called `abc`. It should look something like this (be sure to to add `library(tidyverse)` or `library(readr)` to the script so you can use the `read_csv` function):

```r
abc <- read_csv("abc.csv")
```

Place this line in your app as the second line, just after `library(shiny)` and `library(tidyverse)`.  Make sure the file path and file name are correct, otherwise your app won't run. Try to run the app to make sure the file can be loaded without errors.
...
[![Shiny layout](images/shiny-layout.png)](images/shiny-layout.png)
...
[![Shiny inputs](images/shiny-inputs.png)](images/shiny-inputs.png)
...
[![Shiny add inputs](images/shiny-addinputs.png)](images/shiny-addinputs.png)
...
[![Shiny add plot](images/shiny-addplot.png)](images/shiny-addplot.png)
...
### Use uiOutput() in our app to populate the subtypes
...
[![Shiny publish](images/shiny-publish.png)](images/shiny-publish.png)
