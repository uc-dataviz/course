...
Statistical transformation functions are called using the `stat_()` syntax.
...
First we want to build the layer for the troop movement. If we just include the troop movement information (ignoring `surviviors` and `direction`), we could write this:
...
> `theme_()` functions contain default settings for individual components of the graph. These do not control the aesthetics/channels as defined in `scale_()` functions. These are static elements of the graphic, or more specifically the non-data elements.
...
```{r axis-limits}
ggplot_build(troops_cities)$layout$panel_ranges[[1]]$x.range
```
...
temps_plot <- ggplot(data = temps, aes(x = long, y = temp)) +
  ...
  labs(x = NULL,
       y = "° Celsius") +
  scale_x_continuous(limits = ggplot_build(troops_cities)$layout$panel_ranges[[1]]$x.range) +
  scale_y_continuous(position = "right") +
  coord_cartesian(ylim = c(-35, 5)) +  # Add some space above/below
  theme_bw(base_family = "sans") +
  theme(panel.grid.major.x = element_blank(),
