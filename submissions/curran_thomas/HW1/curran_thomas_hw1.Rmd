...
#using diplyr to make data tables easier to work with
#set working directory for data ingest
setwd('/Users/TomCurran/Desktop/projects/data_viz/dataviz/assignments/data')

biden_csv<- read.csv('biden.csv')
#adding additional column to group education attainment in genders. After scanning
#the data table, it appears that the maximum education in the sample was college
#therefore, assuming that 0 through 12 was considered 
#biden_tb<-mutate(biden_csv, ed_level="",party_aff="")

#In the given sample, the surveyed individuals are either democratc (dem=1), 
#republican (rep=1) or neither (dem=0 AND rep = 0), I created a field that allows
#capture the independent voters

biden_tb<-mutate(biden_csv,
                 party_aff = ifelse(dem==1,"Democrat",
                                    ifelse(rep == 1, "Republican",
                                           ifelse(dem==0 && rep==0, 
                                                  "Independent", 
                                                  "Independent"))))
biden_tb<-mutate(biden_tb,
                 ed_level = ifelse(educ<9,"Less than HS",
                                   ifelse(educ>12, "College Education",
                                          "High School Education")))

biden_tb<-data.table(biden_tb)


write.csv(biden_tb, "biden_table.csv")

party <-c("Democrat" = "Blue", "Independent"="Gray", "Republican"="Red")
################################################################################
```
```{r echo=FALSE}
################################################################################
#Only female voting data:
females <-data.table(filter(biden_tb, female == 1& ed_level == "College Education"))
#females plot
female_grid<-ggplot(females, aes(x=age, y=biden, color=factor(party_aff), shape=factor(ed_level)))+
  ...
  scale_shape_discrete(name = 'Education Level:',
                       breaks=c("College Education","High School Education", "Less than HS"),
                       labels=c("College Educated","High School Educated", "Less than HS")) +
  theme(legend.position = "bottom",
        legend.text.align = .5,
        legend.box="vertical",
        legend.title.align = 0,
        plot.title=element_text(vjust=2))+
  labs(title="Survey of Attitude Temperature of Females\n", 
       x = "Female Age\n", 
       y="Biden Temperature\n",
...
#Only male votingdata:
males<-data.table(filter(biden_tb, female == 0 & ed_level == "College Education"))
#Males plot
male_grid<-ggplot(males, 
                  aes(x=age, 
                      y=biden, 
                      color=factor(party_aff), 
                      shape=ed_level))+
  geom_jitter(size=3.2)+
  scale_color_manual(values=party) +
  labs(title="Survey of Attitude Temperature of Males\n", x="Male Age \n",
     y="Biden Temperature\n") +
