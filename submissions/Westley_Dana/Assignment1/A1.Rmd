...
test11 <- ggplot(biden, aes(warmth, fill = party)) + 
 geom_bar(aes(y = (..count..)/sum(..count..)), position = "dodge") + scale_y_continuous(labels=percent) + 
...
biden %>% count(party = factor(party), warmth = factor(warmth)) %>% 
    mutate(pct = prop.table(n) * 100) %>% 
    ggplot(aes(x = party, y = n, fill = warmth)) + 
    geom_bar(stat = 'identity', position = 'dodge') + 
...
fixing percentages by group (http://stackoverflow.com/revisions/40250493/2)
