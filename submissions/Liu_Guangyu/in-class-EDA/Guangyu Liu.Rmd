...
```
School of larger share of first generation students have students left school with fewer debt. 
    
*Is that because the type of schools?*    
    
```{r}
ggplot(scorecard) +
  geom_histogram(mapping = aes(x = cost))
scorecard$cost <- cut(scorecard$cost, 
                     breaks = c(-Inf, 10000, 20000, 30000, 40000, 50000, 60000, Inf), 
                     right = FALSE)
ggplot(scorecard, mapping = aes(x = firstgen, y = debt)) +
  geom_boxplot(aes(group = cut_width(firstgen, 0.1))) + 
  facet_grid(~ cost) +
